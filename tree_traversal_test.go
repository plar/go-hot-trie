...
// Ported from github.com/plar/go-adaptive-radix-tree tree_traversal_test.go.
// Structure statistics are adapted to HOT's deterministic node counts.
...
// Deterministic HOT structure statistics for the bundled datasets.
var (
	wordsStats = treeStats{235886, 5012, 6364, 3762}
	uuidStats  = treeStats{100000, 931, 3620, 0}
	hskStats   = treeStats{4995, 65, 202, 0}
)
...
func TestTreeTraversalPreordered(t *testing.T) {
	...
	assert.Equal(t, Node8, traversal[0].Kind())
	assert.Equal(t, asNode(tree.root), traversal[0])
	...
	tree.ForEach(func(node Node) bool {
		assert.Equal(t, Node8, node.Kind())

		return true
	}, TraverseNode)
}

func TestTreeTraversal48(t *testing.T) {
	t.Parallel()
	...
	// Ensure all leaves are traversed in order.
	assert.Equal(t, 48, tree.size)
	assert.Len(t, leaves, 48)
	assert.Positive(t, nodes)

	for i := 1; i <= 48; i++ {
		assert.Equal(t, Key{byte(i)}, leaves[i-1].Key())
		assert.Equal(t, Leaf, leaves[i-1].Kind())
	}
}
...
func TestTreeIterateWordsStats(t *testing.T) {
	...
	assert.Equal(t, treeStats{235886, 0, 0, 0}, stats)

	stats = collectStats(tree.Iterator(TraverseNode))
	assert.Equal(t, treeStats{0, wordsStats.node8Count, wordsStats.node16Count, wordsStats.node32Count}, stats)
...
func TestIteratorStatsWithReverse(t *testing.T) {
	...
	// Single-byte keys have at most 8 discriminative bits per node, so all
	// compound nodes are Node8; the node count grows by splitting.
	...
	for leafCount, nodeCount := range testCases {
		tree := New()
...
func TestReverseIteratorMissingIndexZero(t *testing.T) {
	t.Parallel()

	tree := New()

	// Insert 17 keys (0..16); the reverse traversal must produce all of
	// them, including key zero, in descending order.
	var expected []int

	for i := range 17 {
		tree.Insert(Key{byte(i)}, i)
	}
	// The expected reverse sequence: [16, 15, ..., 0]
	for i := 16; i >= 0; i-- {
